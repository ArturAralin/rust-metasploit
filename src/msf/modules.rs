...
pub async fn info(
  ...
  moduletypestr: &str,
  modulenamestr: &str,
) -> Result<res::modules::info, MsfError> {
...
impl compactible {
  ...
  pub async fn payload(&self) -> Result<Vec<String>, MsfError> {
    ...
    let mut buf = vec![];
    let mut se = Serializer::new(&mut body);
    let byte = req::modules::compactible(
      "module.compatible_payloads".to_string(),
      self.client.token.as_ref().unwrap().to_string(),
      self.name.clone(),
    );
    byte.serialize(&mut se).unwrap();
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::modules::compactible_payloads, derror> =
  ...
  pub async fn target_payloads(&self, targetindx: i32) -> Result<Vec<String>, MsfError> {
    ...
    let mut buf = vec![];
    let mut se = Serializer::new(&mut body);
    let byte = req::modules::compactible_tp(
      "module.target_compatible_payloads".to_string(),
      self.client.token.as_ref().unwrap().to_string(),
      self.name.clone(),
      targetindx,
    );
  ...
  pub async fn sessions(&self) -> Result<Vec<String>, MsfError> {
    ...
    let mut se = Serializer::new(&mut body);
    let byte = req::modules::compactible(
      "module.compatible_sessions".to_string(),
      self.client.token.as_ref().unwrap().to_string(),
      self.name.clone(),
    );
    byte.serialize(&mut se).unwrap();
...
pub async fn option(
  ...
  moduletypestr: &str,
  modulenamestr: &str,
) -> Result<HashMap<String, res::modules::options>, MsfError> {
  let moduletype: String = moduletypestr.to_string();
  let modulename: String = modulenamestr.to_string();
  ...
  match con {
    Ok(_) => {
      let de_ret: Result<HashMap<String, res::modules::options>, derror> =
        Deserialize::deserialize(&mut de);
      if let Err(_) = de_ret {
        let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
        test = Err(de_ret);
