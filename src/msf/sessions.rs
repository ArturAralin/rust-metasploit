...
impl shell {
  ...
  pub async fn read(
    client: Client,
    sessionidstr: &str,
    readpointer: Option<i32>,
  ) -> Result<res::sessions::shell_read, MsfError> {
    let sessionid: String = sessionidstr.to_string();
    ...
    match readpointer {
      Some(_) => {
        let byte = req::sessions::shell_read_with_pointer(
          "session.shell_read".to_string(),
          client.token.unwrap(),
          sessionid,
          readpointer.unwrap(),
        );
      ...
      None => {
        let byte = req::sessions::shell_read(
          "session.shell_read".to_string(),
          client.token.unwrap(),
          sessionid,
        );
  ...
  pub async fn write(
    ...
    let mut buf = vec![];
    let mut se = Serializer::new(&mut body);
    let byte = req::sessions::shell_write(
      "session.shell_write".to_string(),
      client.token.unwrap(),
      sessionid,
      data,
    );
...
impl meterpreter {
  ...
  fn serialize(&self, body: &mut Vec<u8>, method: &str, param: Option<String>) {
    ...
    match param {
      Some(val) => {
        let byte = req::sessions::meterpreter_with_two(
          method.to_string(),
          self.client.token.as_ref().unwrap().to_string(),
      ...
      None => {
        let byte = req::sessions::meterpreter_with_one(
          method.to_string(),
          self.client.token.as_ref().unwrap().to_string(),
          self.sessionid.clone(),
        );
  ...
  pub async fn write(&self, datastr: &str) -> Result<bool, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_write, derror> =
  ...
  pub async fn read(&self) -> Result<String, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_read, derror> =
          Deserialize::deserialize(&mut de);
        if let Err(_) = de_ret {
          let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
          test = Err(de_ret);
  ...
  pub async fn run_single(&self, commandstr: &str) -> Result<bool, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_run_single, derror> =
          Deserialize::deserialize(&mut de);
        if let Err(_) = de_ret {
          let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
          test = Err(de_ret);
  ...
  pub async fn script(&self, scriptnamestr: &str) -> Result<bool, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_script, derror> =
          Deserialize::deserialize(&mut de);
        if let Err(_) = de_ret {
          let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
          test = Err(de_ret);
  ...
  pub async fn detach_session(&self) -> Result<bool, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_session_detach, derror> =
          Deserialize::deserialize(&mut de);
        if let Err(_) = de_ret {
          let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
          test = Err(de_ret);
  ...
  pub async fn kill_session(&self) -> Result<bool, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_session_kill, derror> =
          Deserialize::deserialize(&mut de);
        if let Err(_) = de_ret {
          let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
          test = Err(de_ret);
  ...
  pub async fn tabs(&self, inputlinestr: &str) -> Result<Vec<String>, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::meterpreter_tabs, derror> =
          Deserialize::deserialize(&mut de);
        if let Err(_) = de_ret {
          let de_ret: MsfError = from_read(new_buf.as_slice()).unwrap();
          test = Err(de_ret);
  ...
  pub async fn compactible_modules(&self) -> Result<Vec<String>, MsfError> {
    ...
    match con {
      Ok(_) => {
        let de_ret: Result<res::sessions::compactible_modules, derror> =
          Deserialize::deserialize(&mut de);
        if let Ok(ref val) = de_ret {
...
pub async fn shell_upgrade(
  ...
  let mut buf = vec![];
  let mut se = Serializer::new(&mut body);
  let byte = req::sessions::shell_upgrade(
    "session.shell_upgrade".to_string(),
    client.token.as_ref().unwrap().to_string(),
    sessionid,
    connecthost,
    connectport,
  );
...
impl ring {
  ...
  fn serialize(&self, body: &mut Vec<u8>, method: &str, arg: Option<String>) {
    ...
    match arg {
      Some(val) => {
        let byte = req::sessions::ring_with_arg(
          method.to_string(),
          self.client.token.as_ref().unwrap().to_string(),
          self.sessionid.clone(),
          val,
      ...
      None => {
        let byte_new = req::sessions::ring_without_arg(
          method.to_string(),
          self.client.token.as_ref().unwrap().to_string(),
          self.sessionid.clone(),
        );
