...
pub async fn logout(clientdata: client::Client) -> Result<bool, MsfError> {
  ...
  let byte = req::auth::logout(
    "auth.logout".to_string(),
    clientdata.token.as_ref().unwrap().to_string(),
    clientdata.token.as_ref().unwrap().to_string(),
  );
...
pub async fn add_token(clientdata: client::Client, newtokenstr: &str) -> Result<bool, MsfError> {
  ...
  let mut buf = vec![];
  let mut serializer = Serializer::new(&mut body);
  let byte = req::auth::tokenadd(
    "auth.token_add".to_string(),
    clientdata.token.as_ref().unwrap().to_string(),
    new_tok,
  );
  byte.serialize(&mut serializer).unwrap();
...
pub async fn generate_token(clientdata: client::Client) -> Result<String, MsfError> {
  ...
  let mut body = Vec::new();
  let mut serializer = Serializer::new(&mut body);
  let byte = req::auth::tokengen(
    "auth.token_generate".to_string(),
    clientdata.token.as_ref().unwrap().to_string(),
  );
  byte.serialize(&mut serializer).unwrap();
  let mut buf = vec![];
...
pub async fn remove_token(clientdata: client::Client, tokenremove: &str) -> Result<bool, MsfError> {
  ...
  let mut buf = vec![];
  let mut serializer = Serializer::new(&mut body);
  let byte = req::auth::tokenrem(
    "auth.token_remove".to_string(),
    clientdata.token.unwrap(),
    token_rem,
  );
  byte.serialize(&mut serializer).unwrap();
  let con = connect::connect_async(clientdata.url, body, &mut buf).await;
